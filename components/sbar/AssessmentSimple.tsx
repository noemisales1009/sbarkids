...
const AssessmentSimple: React.FC<AssessmentSimpleProps> = ({
  ...
  roundId,
  currentUserName,
  selectedShift: controlledShift,
  onShiftChange,
  onSaved
}) => {
  const [internalSelectedShift, setInternalSelectedShift] = useState<ShiftType>('morning');
  const selectedShift = controlledShift ?? internalSelectedShift;
  const setSelectedShift = (shift: ShiftType) => {
    if (onShiftChange) onShiftChange(shift);
    else setInternalSelectedShift(shift);
  };
