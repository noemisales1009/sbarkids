...
const AssessmentHistoryPanel: React.FC<AssessmentHistoryPanelProps> = ({ shift, roundId, shiftStatus }) => {
  ...
  if (assessmentLogs.length === 0) {
    return <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-800 rounded text-center text-gray-500 dark:text-gray-400 italic">Nenhuma avaliação salva para este turno</div>;
  }
  ...
  return (
    <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900 rounded-lg border border-blue-200 dark:border-blue-700">
      <h4 className="font-semibold text-blue-900 dark:text-blue-100 mb-3">📋 Histórico de Avaliações - {shiftLabel}</h4>
      <div className="space-y-2">
        {assessmentLogs.map((log, idx) => (
          <div key={idx} className="p-3 bg-white dark:bg-gray-800 rounded border border-blue-100 dark:border-blue-800">
            <div className="flex justify-between items-start">
              <div>
                <p className="font-medium text-gray-900 dark:text-white">
                  ✅ Salvo por: <span className="text-blue-600 dark:text-blue-400">{log.saved_by_name || 'Sistema'}</span>
                </p>
